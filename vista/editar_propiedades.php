...
<head>
    ...
    <title>Editar propiedades del producto Gstore</title>
</head>
<body>
    ...
    <section class="container">
        ...
        <!-- Formulario con propiedades a editar -->
        <div class="text-light">
            <!-- Se selecciona el producto obtenido mediante el método POST en editar.php -->
            <?php
                $mysql = conexionSQL();
                $producto = $_POST['producto'];
                $q = "SELECT * FROM productos WHERE producto='$producto'";
                if($Producto = $mysql->query($q)){
                    $p = $Producto->fetch_assoc();
                }
            ?>

            <!-- Se envía información mediante el método POST a control/editar.php -->
            <!-- Se envía la id mediante echo en el enlace del form action -->
            <form action="../control/editar.php?id=<?php echo $p['id'] ?>" method="POST" enctype="multipart/form-data">
                <label>Nombre del producto:</label><br>
                <textarea name="producto" id="" cols="50" rows="2"><?php print($p['producto']); ?></textarea><br>
                <label>Descripcion:</label><br>
                <textarea name="descripcion" id="" cols="50" rows="4"><?php print($p['descripcion']); ?></textarea><br>
                <label>Tipo:</label><br>
                <textarea name="tipo" id="" cols="10" rows="1"><?php print($p['tipo']); ?></textarea><br>
                <label>Precio:</label><br>
                <textarea name="precio" id="" cols="10" rows="1"><?php print($p['precio']); ?></textarea><br>
                <label>Imagen:</label><br>
                <input type="file" name="imagen"><br>
                <label>Detalles:</label><br>
                <textarea name="detalles" id="" cols="50" rows="10"><?php print($p['detalles']); ?></textarea><br>
                <br>
                <button class="btn btn-primary" type="submit" value="Aceptar">Aceptar</button>
            </form>
        </div>
    </section>
