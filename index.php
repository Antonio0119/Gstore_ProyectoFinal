...
<body>
    ...
    <section>
        <div id="carrusel" class="carousel slide" data-ride="carousel">
            ...
            <!-- Obtención de imagenes de la base de datos para el carrusel (Tabla carrusel) -->
                <?php
                    $mysql = conexionSQL();
                    $q1 = "SELECT * FROM carrusel";
                    if($Carrusel = $mysql->query($q1)){
                        // El contador muestra los 4 primeros banner
                        $counter = 0; 
                        while($p = $Carrusel->fetch_assoc()){
                            if($counter==0){
                            ?>
                            <div class="carousel-item active">
                            <?php
                                print('<img src="'.$p['imagen'].'" alt="imagen" style="width: 100%;">');
                            ?>
                            </div>
                            <?php
                            }
            ...
            <!-- Controles previo y siguiente del carrusel -->
    ...
    <section class="container">
        <!-- CONSOLAS -->
        <div>
            <h2><a href="consolas.php" class="nav-link text-light">Consolas</a></h2>
            <div class="row justify-content-center">

            <!-- Cartas con algunos productos tipo consola -->
                <?php
                // Conexión con la db y obtención de productos tipo consola
                    $mysql = conexionSQL();
                    $q = "SELECT * FROM productos WHERE tipo='consola'";
        ...
        <!-- JUEGOS -->
        <div>
            <h2><a href="juegos.php" class="nav-link text-light">Juegos</a></h2>
            <div class="row justify-content-center">

            <!-- Cartas con algunos productos tipo juego -->
                <?php
                // Conexión con la db y obtención de productos tipo juego
                    $mysql = conexionSQL(); 
        ...
        <!-- OFERTAS -->
        <div>
            <h2><a href="ofertas.php" class="nav-link text-light">Ofertas</a></h2>
            <div class="row justify-content-center">

            <!-- Cartas con algunos productos tipo oferta -->
                <?php
                // Conexión con la db y obtención de productos tipo oferta
                    $mysql = conexionSQL();
                    $q = "SELECT * FROM productos WHERE tipo='oferta'";
                    if($Productos = $mysql->query($q)){
                        $counter = 0;
                        while($p = $Productos->fetch_assoc()){
                            if($counter<3){
    ...
    <footer>
        <div class="container">
            <span class="logo"><a href="index.php"><img src="img/Gstore.png"  style="display: block;margin-right: auto;margin-left: auto;padding: 50px;"></a></span>
            <nav class="navbar navbar-expand-sm navbar-dark justify-content-center">
                <!-- Links -->
                <ul class="navbar-nav">
                    <li class="nav-item">
